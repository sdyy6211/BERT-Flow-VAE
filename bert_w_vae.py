def bert_w_vae(
      ...
      embedding_values = bertflow_embedding_values[:,:,[0,1,5],:].mean(axis = 2).reshape(-1,64*12).numpy()

      W, mu = compute_kernel_bias(embedding_values)

      bertflow_embedding_values = torch.tensor((embedding_values + mu).dot(W),dtype = torch.float)
