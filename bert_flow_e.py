def bert_flow_e(
      ...
      from transformers import AutoModelForTokenClassification,TokenClassificationPipeline,AutoTokenizer
      ...
      class Encoder(nn.Module):
          ...
          def __init__(self,
              ...
              self.linear = nn.Linear(bert_dim, latent_dim)
        
          def forward(self, embedded, random_ = True):
      ...
      if isinstance(train_index,type(None)):

          vaedataset = VAEDataset(bertflow_embedding_values,topic_model_res)
            
      else:
        
          vaedataset = VAEDataset(bertflow_embedding_values[train_index,:],topic_model_res[train_index,:])
